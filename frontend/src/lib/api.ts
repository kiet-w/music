...
export async function fetchTracks() {
  const res = await fetch(`${API_URL}/songs`, { cache: 'no-store' });
...
export async function fetchTrack(id: string) {
  const res = await fetch(`${API_URL}/songs/${id}`, { cache: 'no-store' }); // Assuming this exists or works with query
...
export async function fetchGoogleDriveFiles(token: string) {
  const res = await fetch(`${API_URL}/google-drive/files?token=${token}`, { cache: 'no-store' });
